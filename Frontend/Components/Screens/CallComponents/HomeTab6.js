...
export default function App({}) {
  ...
  useEffect(() => {
    ...
    socket.on('callEnded', data => {
      // Remove the ended call from activeCalls
      setActiveCalls(prevCalls =>
        prevCalls.filter(call => call.callerId !== data.callerId),
      );
      if (data.callerId === callerId || data.calleeId === callerId) {
        leave();
      }
    });
            ...
            <View
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    onPress={() => {
                      otherUserId.current = item.callerId;
                      setType('OUTGOING_CALL');
                      processCall();
                    }}
                    style={{
