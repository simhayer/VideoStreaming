module ApplicationCable
  class Connection < ActionCable::Connection::Base
    def subscribed
      stream_from "Connection#{params[:userId]}"
      puts "stream_from Connection#{params[:userId]}"
    end
  
    def receive(data)
      begin
        puts "in Connection.rb"
        ...
        #   ActionCable.server.broadcast("channel_#{data['otherUserId']}", rtcMessage: data['rtcMessage'])
          ...
          ActionCable.server.broadcast("Connection#{parsed_data['otherUserId']}", parsed_data.to_json)
        end
